...
DWORD WINAPI PipeMan(LPVOID lParam)
{
    hPipe = CreateNamedPipe(
        "\\\\.\\pipe\\osu!Lyrics", PIPE_ACCESS_OUTBOUND, PIPE_TYPE_MESSAGE | PIPE_WAIT, 1, BUF_SIZE, 0, 1000, NULL);
    while (1)
    {
...
VOID Send(char *lpBuffer, DWORD nNumberOfBytesToWrite)
{
    if (bPipeConnected &&
        !WriteFile(hPipe, lpBuffer, nNumberOfBytesToWrite, NULL, NULL) && GetLastError() == ERROR_BROKEN_PIPE)
    {
        bPipeConnected = FALSE;
    }
}
...
BOOL WINAPI hkReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped)
{
    ...
    {
        auto pair = audioMap.find(string(path)); // [ audio, osu ]
        if (pair != audioMap.end())
        {
            char buff[BUF_SIZE];
            sprintf(buff, "%s|%d|%s\n", &path[4], SetFilePointer(hFile, 0, NULL, FILE_CURRENT) - *lpNumberOfBytesRead, &pair->second[4]);
